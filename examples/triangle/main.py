...
import kube.vkabstractapplication
...
def surface_win32():
    ...
    surface_create = VkWin32SurfaceCreateInfoKHR(
        ...
        hinstance=get_instance(kube.vkabstractapplication.win.window),
        hwnd=kube.vkabstractapplication.win.window,
        flags=0,
    )
...
print(
    "availables devices: %s"
    % [p.deviceName for p in physical_devices_properties.values()]
)
print("selected device: %s\n" % physical_devices_properties[physical_device].deviceName)

# endregion

# region Select queue family
...
for i, queue_family in enumerate(queue_families):
    ...
    support_present = vkGetPhysicalDeviceSurfaceSupportKHR(
        physicalDevice=physical_device, queueFamilyIndex=i, surface=surface
    )
...
# region Create logical device and queues
...
queues_create = [
    ...
    for i in {queue_family_graphic_index, queue_family_present_index}
]
...
graphic_queue = vkGetDeviceQueue(
    device=logical_device, queueFamilyIndex=queue_family_graphic_index, queueIndex=0
)
presentation_queue = vkGetDeviceQueue(
    device=logical_device, queueFamilyIndex=queue_family_present_index, queueIndex=0
)
...
surface_capabilities = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
    physicalDevice=physical_device, surface=surface
)
surface_formats = vkGetPhysicalDeviceSurfaceFormatsKHR(
    physicalDevice=physical_device, surface=surface
)
surface_present_modes = vkGetPhysicalDeviceSurfacePresentModesKHR(
    physicalDevice=physical_device, surface=surface
)
...
if (
    ...
    and imageCount > surface_capabilities.maxImageCount
):
...
# region Create image view for each image in swapchain
image_views = []
...
for image in image_views:
    ...
    framebuffers.append(vkCreateFramebuffer(logical_device, framebuffer_create, None))
...
for i, command_buffer in enumerate(command_buffers):
    ...
    # Create render pass
    render_area = VkRect2D(offset=VkOffset2D(x=0, y=0), extent=extent)
    ...
    vkCmdBeginRenderPass(
        command_buffer, render_pass_begin_create, VK_SUBPASS_CONTENTS_INLINE
    )
...
def draw_frame():
    try:
        image_index = vkAcquireNextImageKHR(
            logical_device, swapchain, UINT64_MAX, semaphore_image_available, None
        )
    except VkNotReady:
        print("not ready")
        return
